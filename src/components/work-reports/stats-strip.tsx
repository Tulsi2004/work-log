...
import { ClipboardList, Building2, FileText, CalendarClock } from "lucide-react";
import { Card, CardContent } from "@/components/ui/card";
import { Skeleton } from "@/components/ui/skeleton";
import { useDashboardStats } from "@/hooks/use-dashboard";
import { useEmployments } from "@/hooks/use-employments";
import { employmentLabel, formatMonths } from "@/utils/format";
        ...
        <CardContent className="flex items-center justify-between">
          <div>
            ...
            {isLoading ? (
              ...
              <p className="text-2xl font-semibold">{formatMonths(data?.experienceMonths ?? 0)}</p>
            )}
          </div>
          <CalendarClock className="size-7 text-muted-foreground/40 sm:size-8" />
        </CardContent>
      </Card>
    </div>
  );
